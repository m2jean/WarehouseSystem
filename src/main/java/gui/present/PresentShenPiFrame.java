...
public class PresentShenPiFrame extends JFrame implements AddProductInterface {

	private static final long serialVersionUID = -3375976826224663967L;
	private JTable table;
	private PresentShenPiFrame thisFrame;
	...
	public PresentShenPiFrame(PresentVO present) {
		this.present = present;
		thisFrame = this;
		
		setTitle(present.getID());
		...
		JButton button = new JButton("通过");
		button.addActionListener(new PassListener());
		panel.add(button);
		
		JButton btnNewButton = new JButton("不通过");
		btnNewButton.addActionListener(new FailListener());;
	...
	public void updatePresent(Status status){
		if(present.getStatus() == Status.DRAFT){
			JOptionPane.showMessageDialog(null, "草稿单不能审批", "错误", JOptionPane.ERROR_MESSAGE);
		}else if(present.getStatus() != Status.SUBMIT){
			JOptionPane.showMessageDialog(null, "已审批过的单据不能再次审批", "错误", JOptionPane.ERROR_MESSAGE);
			return;
		}
		String date = label_date.getText();
		String operator = label_name.getText();
		...
		if(list.size() == 0){
			...
			PresentVO updPresent = new PresentVO(date, present.getID(), list, operator, status);
			InventoryBLService inventoryService = new InventoryBL();
			ResultMessage message;
			message = inventoryService.updatePresentTable(updPresent);
			
			if(message == ResultMessage.SUCCESS){
				JOptionPane.showMessageDialog(null, "审批成功", "提示", JOptionPane.INFORMATION_MESSAGE);
				dispose();
				MainFrame.mf.pnl_docsAdmin.buildList();
	...
	class PassListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			updatePresent(Status.PASS);
		}	
	}
	
	class FailListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			updatePresent(Status.FAIL);
		}	
	}
